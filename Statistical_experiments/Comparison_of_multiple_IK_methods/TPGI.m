function [q,exitflag,t,k]=TPGI(Td,q0,para)
% Input
% Td: desired end pose;
% q0: initial guess;
...
t_tpgi=tic;
exitflag=0;
% Parameters of TPGI solver
max_time=50e-3;    % Max runtime
k_max=1000;    % Max number of iterations
eps_po=1e-6;    % Error threshold
overrun_threshold=2;    % Threshold of the number of consecutive joint overruns

qlim=para.qlim;    % Joint limits
pt=Td(1:3,4);
et=Td(1:3,3);
...
overlimit_flag=zeros(6,k_max/2);
k=0;
t=999;
N=zeros(3,5);
N(:,1)=[0;0;d1];
x0=[1;0;0]; y0=[0;1;0]; z0=[0;0;1];
% Length of equivalent links
...
% Unit vector of equivalent links
e(:,1)=z0; e(:,2)=x2; e(:,3)=x4;
e(:,4)=z6; e(:,5)=z10;
% Position of equivalent joints
for i=2:5
    N(:,i)=N(:,i-1)+e(:,i)*l(i);
end

while toc(t_tpgi)<max_time/2
    %% If converge
    dp=N(:,5)-pt;
    do=acos(dot(e(:,5),et));
    epo=[dp;do];
